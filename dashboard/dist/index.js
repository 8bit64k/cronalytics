(function () {
  ...
  // ── Sparkline: cost bars + tokens line + duration line ────────────
  function SparkLine({ runs }) {
    ...
    const maxTok  = Math.max(...chrono.map(r => (r.input_tokens || 0) + (r.output_tokens || 0) + (r.cache_read_tokens || 0) + (r.cache_write_tokens || 0)), 1);
    ...
    const tokPts  = chrono.map((r, i) => `${cx(i)},${cy((r.input_tokens||0)+(r.output_tokens||0)+(r.cache_read_tokens||0)+(r.cache_write_tokens||0), maxTok)}`).join(" ");
    ...
    return React.createElement("div", { style: { marginBottom: "0.5rem", position: "relative" } },
      ...
      React.createElement("div", {
        ...
        React.createElement("span", null, "— cost (bar)", " ", "·", " ", React.createElement("span", { style: { color: "#60a5fa" } }, "— tokens"), " ", "·", " ", React.createElement("span", { style: { color: "#fcd34d" } }, "- - duration"))
      ),
      ...
      }, fmtTime(hoverRun.run_time) + " · " + fmtCost(hoverRun.estimated_cost_usd) + " · " + fmtCompact((hoverRun.input_tokens||0)+(hoverRun.output_tokens||0)+(hoverRun.cache_read_tokens||0)+(hoverRun.cache_write_tokens||0)) + " toks · " + fmtDuration(hoverRun.duration_seconds) + " · " + shortModel(hoverRun.model))
    );
  }
